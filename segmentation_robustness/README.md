...
Current project status: this directory remains the standalone segmentation robustness harness. The active frequency-alignment project integrates segmentation as optional Exp 6 under `frequency_alignment/`, where PartImageNet GT masks are used for clean and perturbed mIoU and SAM2 acts as the vision-only control. See `../frequency_alignment/experiment_run.md` for the current Exp 6 operational details and `../full_project_explanation.md` §15 for the scientific framing.
