...
def fft_filter_keep(channels: torch.Tensor, mode: str, cutoff: float) -> torch.Tensor:
    """Keep only low/high frequency band of channels."""
    device = channels.device
    dtype = channels.dtype
    _, _, h, w = channels.shape
    mask = make_frequency_mask(h, w, mode, cutoff, device, torch.float32)
    while mask.dim() < channels.dim():
        mask = mask.unsqueeze(0)
    filt = torch.fft.ifft2(torch.fft.fft2(channels.float(), dim=(-2, -1)) * mask, dim=(-2, -1)).real
...
def parse_float_list(val: Optional[str], default: list[float] | None = None) -> list[float]:
    if not val:
        return default or []
    out: list[float] = []
    for part in val.split(","):
        ...
        try:
            if "/" in part:
                out.append(float(Fraction(part)))
            else:
...
def evaluate_channels(
    ...
    tokens = attack_channels_to_tokens(channels, frames, image_grid, processor)
    inputs = dict(token_inputs)
    inputs["pixel_values"] = tokens
    outputs = model(**inputs)
    log_probs = torch.log_softmax(outputs.logits[:, :-1, :], dim=-1)
    scores = compute_loglik_scores(
        log_probs,
    ...
    margin = float("nan")
    if base_label in scores and scores:
        others = [v for k, v in scores.items() if k != base_label]
        if others:
            margin = float(scores[base_label] - max(others))
    return pred, {k: float(v) for k, v in scores.items()}, margin
...
def run_ablation_experiment(
    ...
    hp_results = []
    for c in cutoffs:
        lp = fft_filter_keep(base_channels, "low", c).clamp(-1.0, 1.0)
        hp = fft_filter_keep(base_channels, "high", c).clamp(-1.0, 1.0)
        for name, chans, bucket in [("lp", lp, lp_results), ("hp", hp, hp_results)]:
            pred, scores, margin = evaluate_channels(
                chans,
                frames,
                image_grid,
                token_inputs,
            ...
            bucket.append({"cutoff": c, "pred": pred, "scores": scores, "margin": margin})

    def flip_threshold(records: List[dict]) -> Optional[float]:
        for rec in records:
            if rec["pred"] is not None and rec["pred"] != base_label:
                return float(rec["cutoff"])
        return None

    lp_flip = flip_threshold(lp_results)
    hp_flip = flip_threshold(hp_results)

    # Plot margins
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(cutoffs, [r["margin"] for r in lp_results], label="LP margin", color="blue")
    ax.plot(cutoffs, [r["margin"] for r in hp_results], label="HP margin", color="red")
    if lp_flip is not None:
        ax.axvline(lp_flip, color="blue", linestyle="--", linewidth=1, label="LP flip")
    if hp_flip is not None:
        ax.axvline(hp_flip, color="red", linestyle="--", linewidth=1, label="HP flip")
    ax.set_xlabel("Cutoff")
    ax.set_ylabel("Margin (base - best other)")
    ax.set_title("Ablation margins")
    ax.legend()
    fig.tight_layout()
    fig.savefig(sample_dir / "ablation_margin.png", dpi=150)
    plt.close(fig)

    # Save representative filtered images
    reps = sorted(set([cutoffs[0], cutoffs[len(cutoffs) // 2], cutoffs[-1]]))
    for c in reps:
        lp = fft_filter_keep(base_channels, "low", c).clamp(-1.0, 1.0)
        hp = fft_filter_keep(base_channels, "high", c).clamp(-1.0, 1.0)
        attack_channels_to_pil(lp, frames).save(sample_dir / f"lp_c{c:.2f}.png")
        attack_channels_to_pil(hp, frames).save(sample_dir / f"hp_c{c:.2f}.png")

    return {
        "cutoffs": [float(c) for c in cutoffs],
        "lowpass_keep": lp_results,
        "highpass_keep": hp_results,
        "flip_threshold_lowpass_keep": lp_flip,
        "flip_threshold_highpass_keep": hp_flip,
    }


def run_random_noise_experiment(
    base_channels: torch.Tensor,
    frames: int,
    image_grid: torch.Tensor,
    token_inputs: Dict[str, torch.Tensor],
    ...
    results = {"eps_list": [float(e) for e in eps_list], "trials": trials, "low": {}, "high": {}, "all": {}}
    results["freq_cutoff"] = freq_cutoff
    device = base_channels.device
    dtype = base_channels.dtype
    for mode in ("low", "high", "all"):
        flip_rates = []
        margin_means = []
        margin_stds = []
        mask = None if mode == "all" else frequency_mask_for_mode(mode, base_channels, freq_cutoff, device, dtype)
        for eps in eps_list:
            flips = 0
            margins: List[float] = []
            for _ in range(trials):
                noise = torch.randn_like(base_channels)
                if mask is not None:
                    noise = fft_project(noise, mask)
                noise = noise / (noise.abs().max() + 1e-8)
                noisy = (base_channels + eps * noise).clamp(-1.0, 1.0)
                pred, scores, margin = evaluate_channels(
                    noisy,
                    frames,
                    image_grid,
                ...
                if pred is not None and pred != base_label:
                    flips += 1
                if not math.isnan(margin):
                    margins.append(margin)
            flip_rates.append(flips / max(1, trials))
            margin_means.append(float(np.mean(margins)) if margins else float("nan"))
            margin_stds.append(float(np.std(margins)) if margins else float("nan"))
        results[mode] = {"flip_rate": flip_rates, "margin_mean": margin_means, "margin_std": margin_stds}

    # Plots
    fig, ax = plt.subplots(figsize=(6, 4))
    for mode, color in [("low", "tab:blue"), ("high", "tab:red"), ("all", "tab:green")]:
        if results.get(mode):
            ax.plot(eps_list, results[mode]["flip_rate"], label=mode, marker="o", color=color)
    ax.set_xlabel("eps")
    ax.set_ylabel("Flip rate")
    ax.set_title("Random noise flip rate")
    ax.legend()
    fig.tight_layout()
    fig.savefig(sample_dir / "random_noise_fliprate.png", dpi=150)
    plt.close(fig)
    ...
    for mode, color in [("low", "tab:blue"), ("high", "tab:red"), ("all", "tab:green")]:
        if results.get(mode):
            ax.errorbar(
                eps_list,
                results[mode]["margin_mean"],
                yerr=results[mode]["margin_std"],
                label=mode,
                marker="o",
                color=color,
            )
    ...
    ax.set_title("Random noise margin")
    ax.legend()
    fig.tight_layout()
    fig.savefig(sample_dir / "random_noise_margin.png", dpi=150)
    plt.close(fig)

    return results


def compute_condition_embeddings(
    base_channels: torch.Tensor,
    mode_results: Dict[str, AttackResult],
    ablation_record: Optional[dict],
    random_noise_record: Optional[dict],
    frames: int,
    image_grid: torch.Tensor,
    processor,
    model,
    ...
    def to_feat(ch) -> Optional[torch.Tensor]:
        pil = attack_channels_to_pil(ch, frames)
        inp = processor.image_processor(images=[pil], return_tensors="pt")
        pixels = inp["pixel_values"].to(device=device, dtype=next(model.parameters()).dtype)
        grid = inp.get("image_grid_thw")
        feat = model.get_image_features(pixel_values=pixels, image_grid_thw=grid)
        emb = feat
        if isinstance(emb, (list, tuple)):
            emb = next((x for x in emb if torch.is_tensor(x)), emb[0] if emb else None)
        if isinstance(emb, dict):
            emb = next((v for v in emb.values() if torch.is_tensor(v)), None)
    ...
    clean_emb = to_feat(base_channels)
    if clean_emb is None:
        return {"vision_cosine": {}}
    norms = clean_emb.norm(dim=-1, keepdim=True) + 1e-8
    clean_normed = clean_emb / norms

    vision_cos = {"clean": 1.0}

    def add_cos(name: str, ch: torch.Tensor):
        emb = to_feat(ch)
        if emb is None:
            return
        emb = emb / (emb.norm(dim=-1, keepdim=True) + 1e-8)
        cos = float(F.cosine_similarity(clean_normed, emb, dim=-1).mean().item())
        vision_cos[name] = cos

    # Ablation endpoints
    if ablation_record:
        cuts = ablation_record.get("cutoffs") or []
        if cuts:
            lp = fft_filter_keep(base_channels, "low", cuts[0]).clamp(-1.0, 1.0)
            hp = fft_filter_keep(base_channels, "high", cuts[-1]).clamp(-1.0, 1.0)
            add_cos(f"ablation_lp_c{cuts[0]:.2f}", lp)
            add_cos(f"ablation_hp_c{cuts[-1]:.2f}", hp)
    # Random noise max eps
    if random_noise_record and random_noise_record.get("eps_list"):
        max_eps = random_noise_record["eps_list"][-1]
        rn_cutoff = random_noise_record.get("freq_cutoff", 0.0)
        for mode in ("low", "high", "all"):
            mask = None if mode == "all" else frequency_mask_for_mode(mode, base_channels, rn_cutoff, base_channels.device, base_channels.dtype)
            noise = torch.randn_like(base_channels)
            if mask is not None:
                noise = fft_project(noise, mask)
            noise = noise / (noise.abs().max() + 1e-8)
            noisy = (base_channels + max_eps * noise).clamp(-1.0, 1.0)
            add_cos(f"rand_{mode}_eps{max_eps:.4f}", noisy)
    # PGD advs
    for mode, res in mode_results.items():
        add_cos(f"pgd_{mode}", res.adv_image)

    return {"vision_cosine": vision_cos}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dataset", type=str, default="seedbench", choices=["seedbench", "mmmu", "pope"], help="Dataset adapter to use (seedbench, mmmu, pope).")
    p.add_argument("--mmmu-split", type=str, default="validation", choices=["validation", "dev", "test"], help="MMMU split to use.")
    p.add_argument("--pope-split", type=str, default="adversarial", choices=["adversarial", "popular", "random", "test"], help="POPE split to use.")
    ...
    p.add_argument("--run-ablation", action="store_true", default=True, help="Run frequency ablation sweeps (default on).")
    p.add_argument("--ablation-cutoffs", type=str, default=None, help="Comma-separated cutoffs (0-1) for ablation sweep.")
    p.add_argument("--ablation-num", type=int, default=10, help="Number of evenly spaced cutoffs if ablation-cutoffs not provided.")
    p.add_argument("--run-random-noise", action="store_true", default=True, help="Run random band-limited noise robustness (default on).")
    p.add_argument("--rand-eps", type=str, default="1/255,2/255,4/255,8/255,16/255", help="Comma-separated eps list for random noise.")
    p.add_argument("--rand-trials", type=int, default=5, help="Trials per eps for random noise.")
    p.add_argument("--pgd-eps-list", type=str, default="8/255", help="Optional comma-separated eps list for PGD sweep.")
    p.add_argument("--save-embeddings", action="store_true", default=True, help="Save cosine to clean vision embeddings for selected conditions (default on).")
    p.add_argument("--loglik-batch-size", type=int, default=4, help="Options batch size per forward pass (<=0 to batch all).")
    return p.parse_args()
...
def run_sample(
    ...
    prompt_data = build_user_prompt(sample, [image], "mcq")
    ...
    labels, messages = build_option_messages(
        image, prompt_data.text, sample.options, user_content=prompt_data.content
    )
    ...
    pgd_eps_list = parse_float_list(args.pgd_eps_list, [args.epsilon]) if args.pgd_eps_list else [args.epsilon]
    ablation_cutoffs = (
        parse_float_list(args.ablation_cutoffs)
        if args.ablation_cutoffs
        else [i / max(1, args.ablation_num - 1) for i in range(args.ablation_num)]
    )

    mode_results: Dict[str, AttackResult] = {}
    pgd_sweep: Dict[str, Dict[str, float | str | bool]] = {}
    for mode in MODES:
        for eps in pgd_eps_list:
            ...
            key = f"{mode}_eps{eps:.5f}"
            pgd_sweep[key] = {
                "adv_label": res.adv_label,
                "flip": res.flip,
                "success": res.success,
                "adv_base_score": safe_float(res.adv_base_score),
                "loglik_drop": safe_float(res.loglik_drop),
            }
            if abs(eps - pgd_eps_list[0]) < 1e-9:
    ...
    if args.run_ablation:
        ablation_record = run_ablation_experiment(
            base_image,
            frames,
            image_grid,
            token_inputs,
    ...
    random_noise_record = None
    if args.run_random_noise:
        rand_eps = parse_float_list(args.rand_eps)
        random_noise_record = run_random_noise_experiment(
            base_image,
            frames,
            image_grid,
            token_inputs,
    ...
    if args.save_embeddings:
        embedding_record = compute_condition_embeddings(
            base_image,
            mode_results,
            ablation_record,
            random_noise_record,
    ...
    record = {
        ...
        "random_noise": random_noise_record,
...
def main() -> None:
    ...
    if args.dataset.lower() == "mmmu":
        single_image = [s for s in samples if len(s.image_paths) <= 1]
        if len(single_image) < len(samples):
            print(f"[info] Filtered MMMU to {len(single_image)} single-image samples (dropped {len(samples) - len(single_image)}).")
    ...
    ablation_thresholds = {"lp": [], "hp": []}
    ablation_flip_counts = {"lp": {}, "hp": {}}
    ablation_tot_counts = {"lp": {}, "hp": {}}
    random_noise_eps: List[float] = []
    random_noise_accum: Dict[str, Dict[int, float]] = {"low": {}, "high": {}, "all": {}}
    random_noise_counts: Dict[str, int] = {"low": 0, "high": 0, "all": 0}
    pgd_sweep_accum: Dict[str, Dict[str, int]] = {}
    embedding_accum: Dict[str, List[float]] = {}
    ...
    for sample in samples:
        ...
        if "ablation" in rec and rec["ablation"]:
            abl = rec["ablation"]
            if abl.get("flip_threshold_lowpass_keep") is not None:
                ablation_thresholds["lp"].append(abl["flip_threshold_lowpass_keep"])
            if abl.get("flip_threshold_highpass_keep") is not None:
                ablation_thresholds["hp"].append(abl["flip_threshold_highpass_keep"])
            base_label = rec.get("base_label")
            # accumulate per-cutoff flips
            for band_key, records in [("lp", abl.get("lowpass_keep", [])), ("hp", abl.get("highpass_keep", []))]:
                for entry in records:
                    c = float(entry.get("cutoff", 0.0))
                    ablation_tot_counts[band_key][c] = ablation_tot_counts[band_key].get(c, 0) + 1
                    pred = entry.get("pred")
                    if pred is not None and base_label is not None and pred != base_label:
                        ablation_flip_counts[band_key][c] = ablation_flip_counts[band_key].get(c, 0) + 1
        if "random_noise" in rec and rec["random_noise"]:
            rn = rec["random_noise"]
            eps_list = rn.get("eps_list", [])
            if eps_list and not random_noise_eps:
                random_noise_eps = eps_list
            for mode in ("low", "high", "all"):
                if mode not in rn:
                    continue
                random_noise_counts[mode] += 1
                for idx, rate in enumerate(rn[mode].get("flip_rate", [])):
                    random_noise_accum[mode][idx] = random_noise_accum[mode].get(idx, 0.0) + rate
        if "pgd_sweep" in rec and rec["pgd_sweep"]:
            for key, vals in rec["pgd_sweep"].items():
                if key not in pgd_sweep_accum:
                    pgd_sweep_accum[key] = {"count": 0, "success": 0, "flip": 0}
                pgd_sweep_accum[key]["count"] += 1
                pgd_sweep_accum[key]["success"] += int(bool(vals.get("success")))
                pgd_sweep_accum[key]["flip"] += int(bool(vals.get("flip")))
        if "embeddings" in rec and rec["embeddings"]:
            vc = rec["embeddings"].get("vision_cosine", {})
            for name, val in vc.items():
                embedding_accum.setdefault(name, []).append(val)
    ...
    if ablation_thresholds["lp"] or ablation_thresholds["hp"]:
        summary["ablation"] = {
            "lp_mean_flip_threshold": float(np.mean(ablation_thresholds["lp"])) if ablation_thresholds["lp"] else None,
            "hp_mean_flip_threshold": float(np.mean(ablation_thresholds["hp"])) if ablation_thresholds["hp"] else None,
            "lp_median_flip_threshold": float(np.median(ablation_thresholds["lp"])) if ablation_thresholds["lp"] else None,
            "hp_median_flip_threshold": float(np.median(ablation_thresholds["hp"])) if ablation_thresholds["hp"] else None,
        }
        # add per-cutoff flip rates
        summary["ablation"]["flip_rates"] = {}
        for band_key in ("lp", "hp"):
            cutoffs_sorted = sorted(ablation_tot_counts[band_key].keys())
            flip_rates = []
            totals = []
            flips = []
            for c in cutoffs_sorted:
                tot = ablation_tot_counts[band_key].get(c, 0)
                flp = ablation_flip_counts[band_key].get(c, 0)
                totals.append(tot)
                flips.append(flp)
                flip_rates.append((flp / tot) if tot else 0.0)
            summary["ablation"]["flip_rates"][band_key] = {
                "cutoffs": cutoffs_sorted,
                "flip_counts": flips,
                "total_counts": totals,
                "flip_rate": flip_rates,
            }
    if any(random_noise_counts.values()):
        summary["random_noise"] = {"low": {}, "high": {}, "all": {}}
        for mode in ("low", "high", "all"):
            if random_noise_counts[mode] == 0:
                continue
            summary["random_noise"][mode] = {
                "flip_rate_mean": [random_noise_accum[mode].get(i, 0.0) / random_noise_counts[mode] for i in sorted(random_noise_accum[mode])],
            }
    if pgd_sweep_accum:
        summary["pgd_sweep"] = {}
        for key, vals in pgd_sweep_accum.items():
            cnt = vals["count"]
            summary["pgd_sweep"][key] = {
                "asr": vals["success"] / cnt,
                "pfr": vals["flip"] / cnt,
                "samples": cnt,
            }
    if embedding_accum:
        summary["embeddings"] = {k: float(np.mean(v)) for k, v in embedding_accum.items()}
    ...
    try:
        if ablation_thresholds["lp"] or ablation_thresholds["hp"]:
            ...
            data = []
            labels_box = []
            if ablation_thresholds["lp"]:
                data.append(ablation_thresholds["lp"])
                labels_box.append("LP")
            if ablation_thresholds["hp"]:
                data.append(ablation_thresholds["hp"])
                labels_box.append("HP")
            ax.boxplot(data, labels=labels_box)
            ax.set_title("Ablation flip thresholds")
            ax.set_ylabel("Cutoff")
            fig.tight_layout()
            fig.savefig(out_dir / "ablation_threshold_boxplot.png", dpi=150)
            plt.close(fig)
        if random_noise_eps:
            fig, ax = plt.subplots(figsize=(6, 4))
            for mode in ("low", "high", "all"):
                if random_noise_counts[mode] == 0:
                    continue
                rates = [random_noise_accum[mode].get(i, 0.0) / random_noise_counts[mode] for i in range(len(random_noise_eps))]
                ax.plot(random_noise_eps, rates, label=mode, marker="o")
            ax.set_xlabel("eps")
            ax.set_ylabel("Mean flip rate")
            ax.set_title("Random noise flip rate (mean)")
            ax.legend()
            fig.tight_layout()
            fig.savefig(out_dir / "random_noise_fliprate_mean.png", dpi=150)
            plt.close(fig)
        if pgd_sweep_accum:
            fig, ax = plt.subplots(figsize=(6, 4))
            for key, vals in pgd_sweep_accum.items():
                ...
                ax.bar(key, asr)
            ax.set_ylabel("ASR")
            ax.set_title("PGD ASR (sweep)")
            fig.autofmt_xdate(rotation=45)
            fig.tight_layout()
            fig.savefig(out_dir / "pgd_asr_mean.png", dpi=150)
            plt.close(fig)
    except Exception as exc:
        print(f"[warn] failed to write global plots: {exc}", flush=True)
    with open(out_dir / "summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    for mode in MODES:
        stats = summary.get(mode, {"asr": 0.0, "pfr": 0.0, "fidelity_mean": None, "samples": 0})
